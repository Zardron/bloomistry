...
export default function HomeClient({
  ...
  const productTabs = useMemo<ProductTab[]>(() => {
    const activeCategories = initialCategories
      .filter((category) => category.isActive)
      .toSorted((first, second) => (first.sortOrder ?? 0) - (second.sortOrder ?? 0));

    return activeCategories.map((category) => {
      const categoryFlowers = initialFlowers
        .filter((flower) => {
          const flowerCategoryId =
            typeof flower.category === "object" ? flower.category._id : flower.category;

          return flower.isActive && flowerCategoryId === category._id && flower.image?.url;
        })
        .map((flower, index) => ({
          key: flower._id,
          src: getApiAssetUrl(flower.image?.url),
          alt: `${flower.name} fuzzy wire bouquet`,
          title: flower.name || `${category.name} #${index + 1}`,
          subtitle: `${category.name} Flower`,
        }));

      return {
        title: category.name,
        price: category.priceLabel || "Ask for pricing",
        description: category.description || `${category.name} Bloomistry bouquet collection.`,
        label: `${category.name} Flower`,
        items: categoryFlowers,
      };
    });
  }, [initialCategories, initialFlowers]);

  const activeTab =
    productTabs.find((tab) => tab.title === activeTabTitle) ?? productTabs[0];
  const activeProductItems = activeTab?.items ?? [];
  const allItemsCount = productTabs.reduce((total, section) => total + section.items.length, 0);
  const featuredFlower =
    initialFlowers.find((flower) => flower.isActive && flower.isFeatured && flower.image?.url) ??
    initialFlowers.find((flower) => flower.isActive && flower.image?.url);
  const featured = featuredFlower
    ? {
        src: getApiAssetUrl(featuredFlower.image?.url),
        alt: `${featuredFlower.name} fuzzy wire bouquet`,
        title: featuredFlower.name,
      }
    : null;

  const customerItems = initialCustomers
    .filter((customer) => customer.isActive && customer.image?.url)
    .map((customer, index) => ({
      key: customer._id,
      src: getApiAssetUrl(customer.image?.url),
      alt: `${customer.customerName || "Bloomistry customer"} photo`,
      title: customer.customerName || "Customer Moment",
            ...
            <div className="relative mx-auto max-w-[620px]">
              ...
              <div className="relative rounded-t-[42vw] border border-[#d1ad51]/65 bg-white p-3 shadow-2xl shadow-[#77669d]/20 sm:rounded-t-full sm:p-4">
                {featured ? (
                  ...
                  <div className="flex aspect-[4/5] w-full items-center justify-center rounded-t-[38vw] bg-[#fbf8ff] px-8 text-center font-serif text-3xl text-[#67558a] sm:rounded-t-full">
                    Loading blooms
                  </div>
                )}
              </div>
              <div className="relative -mt-12 ml-auto w-[90%] bg-white/90 p-4 shadow-xl shadow-[#77669d]/10 backdrop-blur sm:-mt-16 sm:w-[82%] sm:p-5">
                ...
                <p className="mt-2 font-serif text-2xl text-[#67558a] sm:text-3xl">
                  {featured?.title ?? "From the Bloomistry catalog"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-9 flex flex-col justify-between gap-5 border-b border-[#d9c385]/50 pb-7 md:flex-row md:items-end">
            ...
            <p className="max-w-lg text-base leading-7 text-[#625a67]">
              Browse {allItemsCount} flower photos loaded from the Bloomistry catalog.
            </p>
          </div>
            ...
            <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#a98739]">
                  {activeProductItems.length} photos
                </p>
                <h4 className="mt-2 font-serif text-3xl text-[#67558a] sm:text-4xl">
                  {activeTab ? `${activeTab.title} - ${activeTab.price}` : "Loading catalog"}
                </h4>
              </div>
              <p className="max-w-xl text-sm leading-6 text-[#625a67]">
                {catalogError || activeTab?.description || "Fetching the latest catalog items."}
              </p>
            </div>
