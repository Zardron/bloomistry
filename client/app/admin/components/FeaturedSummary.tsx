      ...
      {featuredFlowers.length > 0 ? (
        featuredFlowers.map((flower) => (
          <div key={flower._id} className="border border-[#dfd2ea] bg-[#fbf8ff] p-4">
            ...
            <p className="mt-4 font-serif text-2xl text-[#67558a]">{flower.name}</p>
            <p className="mt-1 text-sm text-[#625a67]">{getFlowerCategoryName(flower)}</p>
            <button
              type="button"
              disabled={isLoading}
              onClick={() => onRemove(flower)}
              className="mt-4 w-full border border-[#c7ad58] bg-white px-4 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[#6f608f] transition hover:bg-[#fffdf7] disabled:opacity-60"
            >
              Remove featured
            </button>
          </div>
